...
export const AppSidebar = () => {
  ...
  const renderMenuItems = (items: typeof mainItems) => <SidebarMenu>
      {items.map(item => <SidebarMenuItem key={item.key}>
          <SidebarMenuButton asChild tooltip={t(item.key)}>
            <NavLink to={item.href} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200", "text-sidebar-foreground/80 hover:text-sidebar-foreground", "hover:bg-sidebar-accent")} activeClassName="bg-sidebar-primary text-sidebar-primary-foreground hover:bg-sidebar-primary hover:text-sidebar-primary-foreground shadow-md">
  ...
  return <Sidebar side={isRTL ? 'right' : 'left'} collapsible="icon" className="border-sidebar-border bg-sidebar transition-all duration-300 fixed">
      {/* ============================================
        ...
        <div className="bg-[sidebar-primary-foreground] bg-[#1a166f] text-[#ee5858] flex-row flex items-center justify-center gap-[12px] border-[#b70606] border-0 rounded shadow opacity-100" dir="ltr">
          ...
          {!isCollapsed && <div className="flex flex-col min-w-0 text-left">
            ...
            direction: 'ltr'
              ...
              <span className="text-xs text-sidebar-foreground/60 truncate" dir={isRTL ? 'rtl' : 'ltr'}>
                {t('salesManagement')}
              </span>
            </div>}
        </div>
      ...
      <SidebarContent className="px-2 py-4 overflow-y-auto">
        {/* Main Menu */}
        <SidebarGroup>
          <SidebarGroupLabel className={cn("text-sidebar-foreground/50 text-xs uppercase tracking-wider mb-2", isRTL && "text-right")}>
        ...
        {filteredOperations.length > 0 && <SidebarGroup className="mt-4">
            <SidebarGroupLabel className={cn("text-sidebar-foreground/50 text-xs uppercase tracking-wider mb-2", isRTL && "text-right")}>
        ...
        {/* Field Operations - IT Admin & Sales Manager */}
        {filteredFieldOps.length > 0 && <SidebarGroup className="mt-4">
            <SidebarGroupLabel className={cn("text-sidebar-foreground/50 text-xs uppercase tracking-wider mb-2", isRTL && "text-right")}>
        ...
        {filteredFinance.length > 0 && <SidebarGroup className="mt-4">
            <SidebarGroupLabel className={cn("text-sidebar-foreground/50 text-xs uppercase tracking-wider mb-2", isRTL && "text-right")}>
        ...
        {filteredAdmin.length > 0 && <SidebarGroup className="mt-4">
            <SidebarGroupLabel className={cn("text-sidebar-foreground/50 text-xs uppercase tracking-wider mb-2", isRTL && "text-right")}>
      ...
      <SidebarFooter className="border-t border-sidebar-border p-3">
        <div className="flex flex-col gap-2">
          ...
          {/* Logout Button */}
          {!isCollapsed && <Button variant="ghost" size="sm" onClick={signOut} className={cn("w-full justify-start gap-2 text-sidebar-foreground/70 hover:text-destructive hover:bg-destructive/10", isRTL && "flex-row-reverse")}>
              <LogOut className="h-4 w-4" />
              <span>{t('logout')}</span>
            </Button>}
          
          <Button variant="ghost" size="sm" onClick={toggleSidebar} className="w-full justify-center text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent">
