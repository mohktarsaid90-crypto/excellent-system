...
<body>
  <div class="header">
    <div class="company-info">
      ...
      <div class="company-details">
        ${companyPhone ? `<div>${isArabic ? 'هاتف' : 'Phone'}: ${escapeHtml(companyPhone)}</div>` : ''}
        ${companyAddress ? `<div>${escapeHtml(companyAddress)}</div>` : ''}
        ${taxId ? `<div>${isArabic ? 'الرقم الضريبي' : 'Tax ID'}: ${escapeHtml(taxId)}</div>` : ''}
      </div>
    </div>
  ...
  <div class="footer">
    <div class="footer-left">
      ${isArabic ? 'إجمالي السجلات' : 'Total Records'}: ${escapeHtml(tableData.rows.length)}
    </div>
